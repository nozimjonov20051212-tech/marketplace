...
// --- SHU QATORNI O'ZGARTIRDIM (RASM CHIQISHI UCHUN ASOSIY YECHIM) ---
...
// --- TA'LIM XIZMATLARI SAHIFASI (YANGI) ---
app.get('/education', (req, res) => {
    const education = getData('educations.json');
    res.render('education', { education, isLoggedIn: req.session.isLoggedIn || false });
});

app.get('/investors', (req, res) => res.render('investors', { isLoggedIn: req.session.isLoggedIn || false }));

// --- KONTAKTLAR YO'NALISHI ---
...
// 4. ADMIN: MA'LUMOT QO'SHISH (BU YERDA CATEGORY QO'SHILDI)
app.post('/admin/add-item', isAdmin, upload.single('image'), (req, res) => {
    const { type, title, description, goal, status, category } = req.body;
    ...
    const newItem = {
        ...
        type: type, // Turi (masalan: education)
        category: category || "all", // Toifasi (masalan: chet-tili)
        title: title || "Nomsiz",
        description: description || "",
        // Agar yuklangan rasm bo'lmasa, fondagi TIQXMMI binosini qo'yamiz
        image: req.file ? '/uploads/' + req.file.filename : '/IMG_9364.JPG',
        goal: goal || "0",
        collected: "0",
        status: status || "Yangi"
    };
    ...
    req.session.save(() => {
        const redirectPath = type === 'education' ? '/education' : '/' + type + 's';
        res.redirect(redirectPath);
    });
});

// --- ADMIN: KONTAKTLARNI TAHRIRLASH ---
...
// --- TA'LIM XIZMATINI O'CHIRISH ---
app.post('/admin/delete/education/:id', isAdmin, (req, res) => {
    let data = getData('educations.json');
    saveData('educations.json', data.filter(i => String(i.id) !== String(req.params.id)));
...
app.listen(PORT, () => console.log(`🚀 Server ishga tushdi: http://localhost:${PORT}`));
>>>>>>> bec3915a8d75b2b7855842e7b1cd3461e2bc096d
